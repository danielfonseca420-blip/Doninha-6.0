...
"""
Gerao de banco de conceitos (L1) a partir do dicionrio em ingls
===================================================================

L o arquivo de texto `data/English dictonary.txt` e constri um
`concepts_en.json` com entradas bsicas:

  - term
  - definition (linha principal do verbete)
  - domain aproximado (quando houver marcadores como 'naut.', 'archit.', etc.)

Este JSON  ento carregado automaticamente por `ConceptTable._load_external_concepts`.
"""
...
def parse_dictionary(path: str) -> List[EnglishConceptEntry]:
    if not os.path.exists(path):
        raise FileNotFoundError(f"Arquivo de dicionrio no encontrado: {path}")
    ...
    with open(path, "r", encoding="utf-8") as f:
        for raw in f:
            ...
            # Ignora cabealhos isolados ('A', 'B', etc.)
            if len(line) == 1 and line.isalpha():
                continue

            # Padro aproximado: "Headword  rest of line"
