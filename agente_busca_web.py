...
=======================================================================
Utiliza LLM Ollama (ReAct), base vetorial local e DuckDuckGo para respostas
precisas, priorizando a base local e complementando com a internet quando necessrio.

Requisitos de ambiente:
  - Ollama rodando localmente com modelo doninha8:latest
  - Base ChromaDB em meu_vector_db (ou configurado em VECTOR_DB_PATH)
  - Dependncias: langchain, langchain-community,
...
# Carregamento de variveis de ambiente
try:
...
# -----------------------------------------------------------------------------
def get_llm():
    """Instancia o ChatOllama com o modelo configurado."""
    if ChatOllama is None:
        raise RuntimeError("Instale langchain e langchain-community para usar o agente de busca.")
    return ChatOllama(
        model=OLLAMA_MODEL,
...
def get_retriever_tool():
    ...
    Retorna None se a base no existir ou se as dependncias no estiverem instaladas.
    """
    ...
    try:
        ...
        return create_retriever_tool(
            ...
            name="busca_local",
            description=(
                "Busca informaes na base de dados local de treinamento. "
...
def get_duckduckgo_tool():
    """Cria a ferramenta DuckDuckGo para busca na web. No requer API key."""
    ...
    try:
        tool = DuckDuckGoSearchRun(
            name="busca_internet",
            description=(
                "Busca informaes atualizadas na internet quando a base local "
                "no tem a resposta ou a confiana  baixa. Use para temas atuais e "
                "consulta a pginas web. No requer chave de API."
            ),
        )
        return tool
    except Exception:
...
# -----------------------------------------------------------------------------
# Prompt do sistema (portugus)
# -----------------------------------------------------------------------------
SYSTEM_PROMPT = """Voc  um agente de pesquisa inteligente.

Regras:
1. Sempre busque primeiro na base local (ferramenta busca_local).
2. Se no encontrar informao suficiente ou a confiana for baixa, use a busca na internet (busca_internet).
3. Responda em portugus, cite fontes e seja preciso.
4. Se precisar de mais informaes, use as ferramentas quantas vezes for necessrio.
5. Ao final, apresente uma resposta clara e bem fundamentada."""


# -----------------------------------------------------------------------------
# Construo do agente ReAct
# -----------------------------------------------------------------------------
def build_agent():
    """
    Monta o agente ReAct com as ferramentas disponveis.
    Usa LangGraph se disponvel; caso contrrio, AgentExecutor clssico.
    """
    llm = get_llm()
    tools = []
    ...
    if local_tool:
        ...
        print(
            "[AVISO] Base ChromaDB no encontrada ou indisponvel. "
            "Apenas busca na internet ser usada.",
            file=sys.stderr,
        )
    ...
    if web_tool:
        ...
        print(
            "[AVISO] DuckDuckGo no disponvel (instale duckduckgo-search). Apenas busca local ser usada.",
            file=sys.stderr,
        )

    if not tools:
        raise RuntimeError(
            "Nenhuma ferramenta disponvel. Configure ChromaDB (meu_vector_db) ou instale duckduckgo-search."
        )

    if USE_LANGGRAPH:
        # LangGraph: create_react_agent retorna um compilado invocvel
        agent = create_react_agent_graph(llm, tools)
        return agent, None, tools

    # LangChain clssico: create_react_agent + AgentExecutor
    ...
    # Prompt no formato ReAct: input + agent_scratchpad; tools/tool_names so preenchidos pelo executor
    prompt = ChatPromptTemplate.from_messages([
        ("system", SYSTEM_PROMPT + "\n\nUse as ferramentas quando necessrio.\n{tools}\n\nNomes das ferramentas: {tool_names}"),
        ("human", "{input}"),
...
# -----------------------------------------------------------------------------
# Invocao unificada (LangGraph ou AgentExecutor)
# -----------------------------------------------------------------------------
def run_agent(query: str, agent_obj, executor, tools):
    """
    Executa o agente com a pergunta do usurio.
    ...
    if USE_LANGGRAPH and agent_obj is not None:
        ...
        # ltima mensagem do assistente  a resposta final
        answer = ""
        steps = []
        sources = []
        for m in messages:
    ...
    # AgentExecutor (LangChain clssico)
    ...
    for step in steps:
        if len(step) >= 2:
            ...
            sources.append({"ferramenta": tool_name, "observao": str(observation)[:500]})
    return answer, steps, sources


# -----------------------------------------------------------------------------
# Funo para uso pelo pipeline (unificao)
# -----------------------------------------------------------------------------
def run_search_for_context(query: str) -> str:
    """
    Executa o agente de pesquisa e retorna resposta + trechos como um nico texto.
    O pipeline agora usa essa etapa automaticamente; qualquer valor False
    configurado para o agente  normalizado para uso ativo.
    """
    try:
        ...
        parts = [answer or ""]
        for s in sources:
            if isinstance(s, dict) and s.get("observao"):
                parts.append(s["observao"][:400])
        return "\n\n".join(parts).strip()
    except Exception:
        return ""
...
# -----------------------------------------------------------------------------
# Main: interao com o usurio
# -----------------------------------------------------------------------------
def main() -> None:
    """Ponto de entrada: pergunta ao usurio, executa o agente e exibe o resultado."""
    print("=" * 60)
    ...
    try:
        ...
        print(f"Erro durante a execuo: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    # Fontes / observaes das ferramentas
    if fontes:
        print("\n" + "-" * 60)
        print("  Fontes / Observaes")
        print("-" * 60)
        for i, f in enumerate(fontes, 1):
            if isinstance(f, dict):
                print(f"  [{i}] {f.get('ferramenta', '')}: {f.get('observao', f)[:300]}...")
            else:
                print(f"  [{i}] {str(f)[:300]}")

    # Passos intermedirios (resumo)
    if passos:
        print("\n" + "-" * 60)
        print("  Passos intermedirios (ReAct)")
        print("-" * 60)
...
# Exemplo de execuo
# -----------------------------------------------------------------------------
# No terminal (com Ollama rodando localmente):
#
#   python agente_busca_web.py
#
# O script pede uma pergunta, executa o agente ReAct (busca local primeiro,
# depois internet se necessrio) e exibe a resposta final, fontes e passos.
# -----------------------------------------------------------------------------
