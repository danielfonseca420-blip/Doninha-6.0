...
Treina o modelo neural de avaliao de verdade (TruthScoringModel) com:

  1) Conjunto de regras do sistema paraconsistente (data/Fuzzy.txt):
     Gc = , Gct = +1, 12 estados, limites Vscc/Vicc/Vscct/Vicct.
     Gera dados sintticos (, )  estado + valor-verdade para treinar L3.

  2) Opcional: documento DOCX com proposies (pr-treinamento fraco).

  - estado lgico: Verdadeiro | Falso | Intermedirio | Indeterminado
...
@dataclass
class TrainConfig:
    backbone_name: str = "cross-encoder/nli-MiniLM2-L6-H768"
    batch_size: int = 16
    num_epochs: int = 3
    learning_rate: float = 2e-5
    max_length: int = 64
    use_fuzzy_rules: bool = True   # Treinar com conjunto de regras do Fuzzy.txt
    fuzzy_grid_step: float = 0.1   # Passo da grade (, ) para dados sintticos
...
def load_training_data_from_fuzzy_rules(
    ...
    Cada ponto (, ) da grade  rotulado com estado e valor-verdade conforme as regras.
    """
    ...
    for mu, lam, state_12, truth in pairs:
        ...
        text = (
            f"Proposio com grau de crena {mu:.2f} e grau de descrena {lam:.2f}. "
            f"Certeza e contradio segundo anlise paraconsistente."
        )
        examples.append(
            PropositionExample(
                text=text,
...
def load_training_data_docx() -> Tuple[List[PropositionExample], List[PropositionExample]]:
    """
    Usa o artigo DOCX como fonte de proposies (pr-treinamento fraco).
    Todas marcadas como Indeterminado com truth_value 0.5.
    """
    if read_docx_file is None:
        raise RuntimeError("corpus_utils.read_docx_file no disponvel.")
    ...
    if not examples:
        raise RuntimeError("Nenhuma sentena extrada do artigo DOCX.")
    split = max(1, int(0.8 * len(examples)))
    train = examples[:split]
    val = examples[split:]
    return train, val
...
def load_training_data(config: TrainConfig) -> Tuple[List[PropositionExample], List[PropositionExample]]:
    """
    Carrega dados de treino: por padro usa o conjunto de regras do Fuzzy.txt (L3 paraconsistente).
...
def train(config: TrainConfig) -> TruthScoringModel:
    ...
    for epoch in range(config.num_epochs):
        ...
        # Validao simples (acurcia de estado)
        model.eval()
        correct, total = 0, 0
        with torch.no_grad():
