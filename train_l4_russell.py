"""
Treino da camada L4 a partir de russell.txt  Base terica de equivalncia
============================================================================
Constri a base de conceitos russellianos (RussellConceptBase) a partir do
...
Conceito central (Russell, The Problems of Philosophy, Cap. XII):
  ...
  - Equivalncia (para a IA) = grau de correspondncia entre a proposio
    refinada (L1L3) e os "fatos" representados no banco de conhecimento.
  - A sntese L4 passa a usar um clculo fundamentado em conceitos
    (correspondncia crenafato) e no somente anlise estatstica.
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Arquivo de sada da base de conceitos (default: l4_russell_concepts.json)",
    )
    ...
    if not os.path.isfile(data_path):
        print(f"Arquivo no encontrado: {data_path}", file=sys.stderr)
        sys.exit(1)

    print("Carregando russell.txt para fundamentar L4 em equivalncia (correspondncia crenafato).")
    ...
    print(f"  Passagens extradas: {len(base.key_passages)}")
